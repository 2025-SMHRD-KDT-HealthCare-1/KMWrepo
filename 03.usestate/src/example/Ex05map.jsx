...
const Ex05map = () => {
      ...
      // 1.map 함수 : 기존 배열을 이용해서 새로운 배열을 만드는 배열함수(JS 내장)
      //           : 기존배열.map(콜백함수)
      // 2.filter함수 : 기존 배열에서 조건을 이용해서 조건이 true면 유지, false면 버려서 새로운 배열을 만드는 배열함수(JS 내장)
      //           : 기존배열.filter(콜백함수)
  ...
  return (
    <div>
        ...
        */}
        {students.map((item) => (
  <div key={item.name}>
    <p>안녕하세요 제 이름은 {item.name}입니다. 저는 {item.bloodType}형이고 {item.age}살입니다.</p>
  </div>
))}

        <h3>case3 : map함수 실습</h3>
        ...
        */}
        {food.map((item) =>(
          ...
          {food.map((item) =>{
          console.log(item);
          return <button style= {{background : 'yellow'}}key = {item} >{item}</button>}
        )}
  <hr></hr>
        ...
        */}
        ...
        {/* 
            ...
            2. 새롭게 만들어진 1,2,3,4,5 버튼 중에서 짝수만 filter함수를 이용해 화면에 출력
        */}
        {/* 
        ...
        */}
          {numList.filter(num => num % 2 === 0).map(
